...
NOTA: Como algunos comentarios de review presentan comillas es necesario hacer una extraccion aparte para esta columna.
...
NOTA: No se presentaron cambios significativos en el código, así que no se comentará este extracto por extracto.
...
NOTA: Si bien muchas frases identificadas en language como que pertenecen a otro idioma en realidad no es así, esto no afecta en nada porque finalmente se van a traducir al inglés.
...
```python
# Para leer los archivos json.gzip
...
# Definir la ventana del SMA (30 días para un mes)
sma_window = 30

# Calcular el SMA del precio utilizando la función rolling
filtered_games['SMA'] = filtered_games['price'].rolling(window=sma_window).mean()

# Crear el gráfico de precios y SMA
